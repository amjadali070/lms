...
const AIGenerator = () => {
  ...
  const handleGenerate = async () => {
    ...
    setError("");
    try {
      const res = await api.post("/ai/generate-course", {
        topic,
        description,
        sector,
        moduleCount,
      ...
      setError(
        err.response?.data?.error ||
          "AI generation failed. Please check your OpenAI API key.",
      );
    } finally {
      setGenerating(false);
    }
      ...
      <div className="bg-primary p-8 rounded-2xl text-white relative overflow-hidden shadow-xl">
        ...
        <div className="relative z-10">
          ...
          <p className="text-accent max-w-2xl">
            Describe any topic and our AI will instantly generate a complete,
            structured course with modules, lessons, and quiz questions ready to
            publish.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white p-8 rounded-xl shadow-sm border border-gray-100 space-y-6">
        ...
        <div>
          ...
          <p className="text-xs text-gray-400 mt-1">
            The more detail you provide, the better the AI will tailor the
            course content.
          </p>
        </div>
            ...
            <div>
              ...
              <div className="flex gap-2 mt-3">
                {(preview.sectors || []).map((s: string) => (
                  <span
                    key={s}
                    className="text-[10px] font-bold uppercase tracking-wider bg-accent/20 text-primary px-2.5 py-1 rounded-md border border-accent/40"
                  >
                    {s}
                  </span>
                ))}
              ...
              >
                <div className="bg-background/50 px-6 py-4 font-bold text-primary flex justify-between">
                  ...
                  <span className="text-sm text-secondary font-medium">
                    {mod.lessons?.length || 0} Lessons • {mod.quiz?.length || 0}{" "}
                    Quiz Questions
                  </span>
                </div>
                <div className="p-4 space-y-2">
                  {mod.lessons?.map((lesson: any, li: number) => (
                    ...
                    >
                      <span
                        className={`text-xs font-bold uppercase px-2 py-0.5 rounded ${lesson.contentType === "video" ? "bg-red-100 text-red-600" : "bg-accent/20 text-primary"}`}
                      >
                        {lesson.contentType}
                      </span>
