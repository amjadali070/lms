  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        ...
        const completed = res.data.filter((c: any) => c.enrollmentStatus === 'completed');
  ...
  const handleDownloadCertificate = async (courseToDownload: any) => {
    ...
    setTimeout(async () => {
      try {
        ...
        pdf.save(`${courseToDownload.title.replace(/\s+/g, '_')}_Certificate.pdf`);
  ...
  return (
    <div className="max-w-full mx-auto space-y-8">
      <div className="bg-white p-8 rounded-xl shadow-sm border border-gray-100 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-primary mb-2">My Certificates</h1>
          <p className="text-gray-600">View and download your official achievements.</p>
        </div>
      ...
      {completedCourses.length === 0 ? (
        <div className="bg-white p-12 text-center rounded-xl border border-gray-100 shadow-sm">
          ...
          <p className="text-gray-500 max-w-md mx-auto">Complete a course to earn your first official certificate.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {completedCourses.map((course) => (
            <div key={course._id} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden flex flex-col group relative">
              ...
              <div className="h-40 bg-primary/5 flex items-center justify-center relative overflow-hidden p-4">
                 <div className="absolute inset-0 opacity-[0.03]" style={{ backgroundImage: 'radial-gradient(#213448 2px, transparent 2px)', backgroundSize: '10px 10px' }}></div>
                 <div className="w-full h-full bg-white shadow-md border border-gray-200 transform scale-[0.6] origin-center rotate-[-2deg] flex items-center justify-center group-hover:rotate-0 transition-transform duration-500 relative">
                   ...
                   <h3 className="text-xs font-serif text-primary text-center px-4 max-w-[80%] whitespace-nowrap overflow-hidden text-ellipsis font-bold">CERTIFICATE OF COMPLETION</h3>
                 </div>
              </div>

              <div className="p-6 flex-1 flex flex-col">
                <span className="text-xs font-bold uppercase tracking-widest text-secondary mb-2 block">Achieved on {new Date(course.completedAt || new Date()).toLocaleDateString()}</span>
                <h3 className="font-bold text-gray-900 text-lg mb-4 line-clamp-2 leading-tight">{course.title}</h3>
