...
const CourseCreate = () => {
  ...
  const [modules, setModules] = useState<any[]>([{ title: '', lessons: [], quiz: [] }]);
  ...
  const availableSectors = ['Government', 'Education', 'Healthcare', 'Transportation', 'Public Administration'];

  useEffect(() => {
    if (courseId) {
      api.get(`/courses/${courseId}`).then(res => {
        const { title, description, sectors, modules, status } = res.data.course;
        setTitle(title);
  ...
  const handleSubmit = async (submitStatus: 'draft' | 'published') => {
    setSubmitting(true);
    try {
      const payload = { title, description, sectors, modules, status: submitStatus };
      let res;
      if (courseId) {
  ...
  const addLesson = (moduleIndex: number) => {
    const newModules = [...modules];
    newModules[moduleIndex].lessons.push({ title: '', contentType: 'video', contentUrl: '', textContent: '', durationMinutes: 10 });
    setModules(newModules);
  };

  const updateLesson = (moduleIndex: number, lessonIndex: number, field: string, val: any) => {
    const newModules = [...modules];
  ...
  const addQuizQuestion = (moduleIndex: number) => {
    ...
    newModules[moduleIndex].quiz.push({ question: '', options: ['', ''], correctAnswer: 0, correctAnswers: [], type: 'single' });
    setModules(newModules);
  };

  const toggleCorrectAnswer = (mIndex: number, qIndex: number, oIndex: number) => {
    const newModules = [...modules];
  ...
  const updateQuizQuestion = (mIndex: number, qIndex: number, field: string, val: any) => {
    const newModules = [...modules];
    newModules[mIndex].quiz[qIndex][field] = val;
    setModules(newModules);
  };

  const updateQuizOption = (mIndex: number, qIndex: number, oIndex: number, val: string) => {
    const newModules = [...modules];
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex justify-between items-center mb-8 border-b border-gray-100 pb-4">
        <div>
          <h1 className="text-3xl font-bold text-primary">{courseId ? 'Edit Course' : 'Draft New Course'}</h1>
          <p className="text-gray-600 mt-1">Design global instructional content</p>
        </div>
        <div className="flex items-center gap-4">
          {status === 'published' ? (
            <span className="bg-green-100 text-green-800 text-xs font-bold px-3 py-1 rounded-full border border-green-200 uppercase tracking-wider">Published</span>
          ) : (
            <span className="bg-yellow-100 text-yellow-800 text-xs font-bold px-3 py-1 rounded-full border border-yellow-200 uppercase tracking-wider">Draft</span>
          )}
          <BookOpen size={48} className="text-background" />
        </div>
      </div>

      <form onSubmit={(e) => e.preventDefault()} className="bg-white p-8 rounded-xl shadow-sm border border-gray-100 space-y-6">
        <div>
          <label className="block text-sm font-semibold text-primary mb-2">Course Title</label>
          <input
            type="text"
            required
            value={title}
        ...
        <div>
          <label className="block text-sm font-semibold text-primary mb-2">Target Sectors</label>
            ...
            {availableSectors.map(sector => (
              <button
                ...
                className={`px-4 py-2 rounded-full text-sm font-medium border transition-colors ${
                  sectors.includes(sector) ? 'bg-secondary text-white border-secondary' : 'bg-white text-gray-600 border-gray-300 hover:border-secondary'
                }`}
              >
                {sector}
              </button>
        ...
        <div className="pt-6 border-t border-gray-100">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-xl font-bold text-primary">Modules & Content Structure</h2>
            <button 
              type="button" 
              onClick={addModule}
          ...
          <div className="space-y-6">
            {modules.map((mod, mIndex) => (
              <div key={mIndex} className="bg-gray-50 p-6 rounded-lg border border-gray-200">
                <div className="flex items-center gap-4 mb-4">
                  <span className="font-bold text-primary w-24">Module {mIndex + 1}:</span>
                  <input
                    type="text"
                    required
                    value={mod.title}
                ...
                <div className="pl-28 space-y-4">
                  <h4 className="text-sm font-bold text-gray-600 mb-2">Lessons</h4>
                  {mod.lessons.map((lesson: any, lIndex: number) => (
                    <div key={lIndex} className="bg-white p-4 border border-gray-200 rounded shadow-sm space-y-3">
                      <div className="flex gap-4">
                        ...
                        <input
                          ...
                          onChange={(e) => updateLesson(mIndex, lIndex, 'durationMinutes', e.target.value)}
                          className="w-24 px-3 py-1 border border-gray-300 rounded focus:ring-secondary focus:border-secondary text-sm"
                          placeholder="Mins"
                        />
                      </div>
                      
                      {lesson.contentType === 'video' || lesson.contentType === 'lab' ? (
                        <input
                          type="text"
                          value={lesson.contentUrl}
                          onChange={(e) => updateLesson(mIndex, lIndex, 'contentUrl', e.target.value)}
                          className="w-full px-3 py-1 border border-gray-300 rounded focus:ring-secondary focus:border-secondary text-sm"
                          placeholder={lesson.contentType === 'video' ? "https://youtube.com/..." : "https://labs.example.com/..."}
                        />
                      ) : (
                        <textarea
                          ...
                          placeholder={lesson.contentType === 'text' ? "Write instructional content here..." : "Describe the assignment constraints..."}
                        />
                      )}
                    </div>
                  ))}
                  ...
                  <div className="pt-6 mt-4 border-t border-gray-200">
                    <h4 className="text-sm font-bold text-gray-600 mb-2">Module Quiz Configuration</h4>
                    {mod.quiz && mod.quiz.map((q: any, qIndex: number) => (
                      <div key={qIndex} className="bg-orange-50/50 p-4 border border-orange-100 rounded mb-4">
                         <div className="flex gap-2 mb-3">
                           <input
                             type="text"
                             value={q.question}
                             onChange={(e) => updateQuizQuestion(mIndex, qIndex, 'question', e.target.value)}
                             className="flex-1 px-3 py-2 border border-orange-200 rounded focus:ring-orange-300 text-sm"
                             placeholder="Question text..."
                           />
                           <select 
                             value={q.type || 'single'}
                             onChange={(e) => updateQuizQuestion(mIndex, qIndex, 'type', e.target.value)}
                         ...
                         <div className="space-y-2 pl-4">
                           {q.options.map((opt: string, oIndex: number) => (
                             <div key={oIndex} className="flex flex-row items-center gap-2">
                               <input 
                                 type={q.type === 'multiple' ? 'checkbox' : 'radio'} 
                                 name={`correct-${mIndex}-${qIndex}`} 
                                 checked={q.type === 'multiple' ? q.correctAnswers?.includes(oIndex) : q.correctAnswer === oIndex}
                                 onChange={() => toggleCorrectAnswer(mIndex, qIndex, oIndex)}
                                 className="text-orange-500 focus:ring-orange-500"
                               />
                               <input
                                 type="text"
                                 value={opt}
                                 onChange={(e) => updateQuizOption(mIndex, qIndex, oIndex, e.target.value)}
        ...
        <div className="pt-6 border-t border-gray-100 flex justify-end gap-4">
           ...
           >
             {submitting ? 'Saving...' : <><CheckCircle size={20} /> Publish Course Layout</>}
           </button>
        </div>
      </form>
    </div>
