...
import { BookOpen, CheckCircle, Video, FileText, Award, ChevronLeft, Edit3, ChevronDown, Check, Download, Eye, X, BarChart } from 'lucide-react';
...
const CourseDetail = () => {
  ...
  if (!course) {
    return <div className="text-center text-red-500 mt-10">Course not found or unauthorized.</div>;
  }
  ...
  return (
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="bg-white p-8 rounded-xl shadow-sm border border-gray-100 relative overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="flex justify-between items-start mb-4">
            <div className="flex items-center gap-4">
              ...
              {['admin', 'instructor'].includes(currentUser?.role || '') && (
                <span className={`text-xs font-bold px-3 py-1 rounded-full border uppercase tracking-wider ${course.status === 'published' ? 'bg-green-100 text-green-800 border-green-200' : 'bg-yellow-100 text-yellow-800 border-yellow-200'}`}>
                  {course.status || 'draft'}
                </span>
              )}
            </div>
          ...
          <div className="flex items-center gap-6 text-sm text-gray-500 font-medium flex-wrap">
            <span className="flex items-center gap-2"><BookOpen size={18} className="text-secondary" /> {course.modules.length} Modules</span>
            <span className="flex items-center gap-2"><Award size={18} className="text-secondary" /> Certificate Included</span>
            {course.sectors && course.sectors.length > 0 && (
              <div className="flex items-center gap-2 flex-wrap">
                {course.sectors.map((sector: string) => (
                  <span key={sector} className="text-[10px] font-bold uppercase tracking-wider bg-blue-50 text-blue-600 px-2.5 py-1 rounded-md border border-blue-100">
                    {sector}
                  </span>
                ))}
              </div>
          ...
          {currentUser?.role === 'student' && !isCompleted && enrollment && (
            <div className="mt-8">
               ...
               >
                 {course.modules.some((m: any) => m.quiz && m.quiz.length > 0) ? 'Take Final Assessment' : 'Complete Course'}
               </button>
            </div>
          )}
          ...
          {isCompleted && currentUser?.role === 'student' && (
            <div className="mt-8 flex flex-wrap gap-4">
               ...
               >
                 <Download size={18} /> {downloadingCert ? 'Generating...' : 'Download'}
               </button>
          ...
          {currentUser?.role === 'student' && enrollment && !isCompleted && (
            <div className="mt-6 border-t border-gray-100/30 pt-4">
               <p className="text-sm text-gray-500 font-medium mb-2">Overall Progress</p>
               <div className="flex items-center gap-4">
                 ...
                 <span className="text-sm font-bold text-primary">{enrollment.progress || 0}%</span>
               </div>
            </div>
          )}
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-8">
        <h2 className="text-2xl font-bold text-primary mb-6 border-b border-gray-100 pb-4">Course Content</h2>
          ...
          {course.modules.map((module: any, index: number) => (
            <div key={module._id || index} className="border border-gray-200 rounded-lg overflow-hidden">
              <div className="bg-gray-50 px-6 py-4 flex items-center justify-between font-bold text-primary">
                ...
                <span className="text-sm text-gray-500 font-medium">{module.lessons.length} Lessons</span>
              </div>
              <div className="divide-y divide-gray-100">
                {module.lessons.map((lesson: any, lIndex: number) => {
                  const lessonCompleted = completedLessons.includes(lesson._id);
                  const isActive = activeLessonId === lesson._id;

                  return (
                    <div key={lesson._id || lIndex} className="flex flex-col">
                      <div 
                        onClick={() => setActiveLessonId(isActive ? null : lesson._id)}
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div className={`p-2 rounded w-8 h-8 flex items-center justify-center ${lessonCompleted ? 'bg-green-100 text-green-500' : 'bg-blue-100 text-blue-500'}`}>
                            {lessonCompleted ? <Check size={16} /> : <Video size={16} />}
                          </div>
                          <span className={`font-medium ${lessonCompleted ? 'text-gray-500 line-through' : 'text-gray-800'}`}>
                            {lesson.title}
                          </span>
                        </div>
                      ...
                      {isActive && (
                        <div className="px-14 py-6 bg-white border-t border-b border-gray-100 animate-fadeIn space-y-4">
                              ...
                              {lesson.contentUrl ? (
                                (() => {
                                  ...
                                  if (finalUrl.includes('youtube.com') || finalUrl.includes('youtu.be')) {
                                    ...
                                    else if (finalUrl.includes('watch?v=')) videoId = finalUrl.split('watch?v=')[1].split('&')[0];
                                  ...
                                  return (
                                    <Player
                                      ...
                                      fallback={<div className="text-white flex items-center justify-center p-8"><Video size={48} className="animate-pulse opacity-50"/></div>}
                                    />
                                  );
                                })()
                              ) : (
                          ...
                          {(lesson.contentType === 'assignment' || lesson.contentType === 'lab') && (
                            <div className="p-6 bg-gray-50 border border-gray-200 rounded-lg">
                              <h4 className="font-bold text-gray-700 mb-2">Instructions:</h4>
                              <p className="text-gray-600 text-sm mb-4">{lesson.textContent}</p>
                              {lesson.contentUrl && (
                                <a href={lesson.contentUrl} target="_blank" rel="noreferrer" className="text-secondary font-medium hover:underline text-sm mb-4 inline-block">
                                  Open External Resource
                                </a>
                              )}
                              
                              {lesson.contentType === 'assignment' && currentUser?.role === 'student' && !lessonCompleted && (
                                <div className="mt-4 pt-4 border-t border-gray-200">
                                   <label className="block text-sm font-semibold text-gray-700 mb-2">Submit your work (PDF, DOCX, ZIP)</label>
                                   <input type="file" className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-primary file:text-white hover:file:bg-secondary cursor-pointer" />
                                </div>
                              )}
                            </div>
                          )}

                          {currentUser?.role === 'student' && !lessonCompleted && (
                            <div className="pt-4 mt-4 border-t border-gray-100 flex justify-end">
                              <button 
                                onClick={() => {
                                  if (lesson.contentType === 'assignment') {
                                    alert('Assignment file uploaded and submitted successfully!');
                                  }
                              ...
                              >
                                {lesson.contentType === 'assignment' ? 'Submit Assignment' : 'Mark as Complete'}
                              </button>
                            </div>
                          )}
                        </div>
                ...
                {module.quiz && module.quiz.length > 0 && (
                  <div className="px-6 py-4 flex items-center justify-between bg-orange-50/30">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="font-medium text-orange-800">Module Quiz ({module.quiz.length} Questions)</span>
                    </div>
                  </div>
                )}
              </div>
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl flex flex-col w-full max-w-[800px] max-h-[95vh] overflow-hidden my-auto border border-gray-100/20">
             ...
             <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-gray-50/80">
               <div className="flex items-center gap-3">
                 ...
                 <h3 className="font-bold text-gray-800 text-lg">Official Certificate</h3>
               </div>
               <button 
             ...
             <div ref={certContainerRef} className="bg-[#f0f2f5] flex items-center justify-center p-6 overflow-hidden flex-1">
                <div 
                ...
                >
                  <div className="absolute top-0 left-0 bg-white ring-1 ring-gray-900/5 origin-top-left" style={{ width: '1123px', height: '794px', transform: `scale(${certScale})` }}>
                    <CertificateTemplate 
                      studentName={currentUser?.name || ''}
                      courseTitle={course.title}
                      issueDate={new Date(enrollment.completedAt || new Date())}
             ...
             <div className="px-6 py-4 bg-gray-50 flex items-center justify-between border-t border-gray-100">
               <span className="text-sm text-gray-500 font-medium hidden sm:block">Ready to print or save.</span>
               <button 
                 onClick={() => {
               ...
               >
                 <Download size={18} /> {downloadingCert ? 'Generating PDF...' : 'Download PDF Copy'}
               </button>
             </div>
