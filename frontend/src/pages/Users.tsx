...
const Users = () => {
  ...
  const [newUser, setNewUser] = useState({ name: '', email: '', password: 'password123', role: 'student' });
  ...
  const handleAddUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setShowModal(false);
      setNewUser({ name: '', email: '', password: 'password123', role: 'student' });
      fetchUsers();
    } catch (err: any) {
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-600 mt-1">Manage system personnel and user accounts.</p>
        </div>
        <button 
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-md relative">
            ...
            <h2 className="text-xl font-bold text-primary mb-4">Add Sector User</h2>
            <form onSubmit={handleAddUser} className="space-y-4">
              ...
              <p className="text-xs text-gray-500 italic mb-4">Default password is 'password123'</p>
              <button 
                type="submit"
